...
function OrderById(props) {
  ...
  const getButtonContent = () => {
    if (order.total < 100000 || order.orderState === "Requesting Approval") {
      return (
        <Button style={{ backgroundColor: "#3b9664" }}
        type="primary"
        onClick={() =>
          showConfirm("Partially approve the order and generate the PO!", generatePO)
        }
      >
        Partially Approve
      </Button>
      );
    } else if (order.total >= 200000 || order.orderState === "Requesting Approval") {
      return (
        <Button style={{ backgroundColor: "#3b9664" }}
          type="primary"
          onClick={() =>
  ...
  const rejectOrder = async () => {
    ...
    props.history.push("/ManageOrders");
  };
  ...
  const generatePO = async () => {
    ...
    items.map((i) => {
      let poRef = doc(db, "PartialAccepted", `PO-${Date.now()}-${count}`);
      batch.set(poRef, {
        orderId: order.id,
        itemId: i.code,
        index: count,
    ...
    <div>
      <Spin spinning={loading}>
        <PageHeader
          ...
          onBack={() => props.history.push("/ManageOrders")}
          title="Order No: "
